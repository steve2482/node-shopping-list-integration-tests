...
describe('Recipes', function () {
  ...
  // 2. Inspect response object and that it has the correct key:value
  // pairs and that the object has an 'id'
  ...
  // test strategy:
  // 1. Make a POST request using a example recipe
  // 2. Inspect the response object and that it has the correct
  // key:value pairs
  it('should POST a new recipe and return that recipe', function() {
    const newRecipe = {name: 'Macaroni & Cheese',ingredients: 'Macaroni, Milk, Cheese, Breadcrumbs'};
    return chai.request(app)
    .post('/recipes')
    .send(newRecipe)
    .then(function(res) {
  ...
  //  has right status code and that we get back an updated
  //  item with the right data in it.
  it('should updated a current recipe', function() {
    // we initialize our updateData here and then after the initial
    // request to the app, we update it with an `id` property so
    // we can make a second, PUT call to the app.
    const updatedRecipe = {
      name: 'foo',
      ingredients: ['bar', 'bizz', 'buzz']
    }
    return chai.request(app)
    // first have to get so we have an idea of object to update
    .get('/recipes')
